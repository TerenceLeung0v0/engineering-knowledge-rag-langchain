...
_debug = build_debug_logger(
    cfg=DEBUG_CONFIG,
    domain_path="rag.ingest",
    key="print_source_indexed"
)
...
def _emit_ingest_diagnostics(chunks: list[Document]) -> None:
    ...
    - source distribution (Controlled via DEBUG_CONFIG["rag"]["ingest"]["print_source_indexed"])
    ...
    _debug(f"Sources indexed ({len(source_counter)}):")
    for source, count in sorted(source_counter.items(), key=lambda x: x[0]):
        _debug(f" - {Path(source).name} ({count} chunks)")
